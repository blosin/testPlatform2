...
const branches = [{
    ...
    "platforms": [
        {
            "progClosed": [{ _id: 'dasdasdadasd', opened: new Date(), closed: new Date() }],
...
describe('Platform', function () {
    ...
    describe('fn(): viewOrder()', function () {
        ...
        it('should view the order correctly', async () => {
            ...
            const stateIdByCodStub = sandbox.stub(NewsStateSingleton, 'stateByCod')
                .withArgs(stateCod)
                .returns(stateId);
            const updOrderStateStub = sandbox.stub(platform, 'updateOrderState')
                .resolves(true)
    ...
    describe('fn(): findOrder()', function () {
        ...
        it('order was found', async function () {
            ...
            sandbox.stub(orderModel, 'findOne')
        ...
        it('order was not found. Catch error.', function () {
            ...
            platform.findOrder(savedOrder.orderId)
                .catch((error) => {
    ...
    describe('fn(): closeRestaurant()', function () {
        ...
        it('should close the restaurant', async function () {
            ...
            const findOneStub = sandbox.stub(branchModel, 'findOne')
                .resolves(branches[0]);
            const updOneStub = sandbox.stub(branchModel, 'updateOne')
                .resolves(null);
            const getBPlatformStub = sandbox.stub(Platform.prototype, 'getBranchPlatform')
                .returns();
            const validateProgClosedStub = sandbox.stub(branchModel, 'validateNewProgClosed')
        ...
        it('should not close the restaurant. It is already close', async () => {
            ...
            try {
                ...
                expect(error).to.eql(`${validatedClosed} RestaurantCode: ${branches[0].branchId}.`);
                expect(findOneStub.callCount).to.equal(1);
                expect(getBPlatformStub.callCount).to.equal(1);
                expect(validateProgClosedStub.callCount).to.equal(1);
            }
    ...
    describe('fn(): openRestaurant()', function () {
        ...
        it('should open the restaurant', async function () {
            ...
            const getBPlatformStub = sandbox.stub(Platform.prototype, 'getBranchPlatform')
                .returns();
            const findProgClosedToOpenStub = sandbox.stub(branchModel, 'findProgClosedToOpen')
        ...
        it('should not open the restaurant. It has not a programmed closed', async () => {
            ...
            platform._platform = { _id: "ssdadsaa" }
            try {
                await platform.openRestaurant(
                    savedOrder.branchId,
                    platformSaved);
            } catch (error) {
                expect(error).to.eql(`Failed to openRestaurant. RestaurantCode: ${savedOrder.branchId}.`);
                expect(findOneStub.callCount).to.equal(1);
                expect(getBPlatformStub.callCount).to.equal(1);
                expect(findProgClosedToOpenStub.callCount).to.equal(1);
            }
    ...
    describe('fn(): updateOrderState()', function () {
        ...
        it('should udpate order state', async function () {
            const findOneAndUpdateStub = sandbox.stub(orderModel, 'findOneAndUpdate')
                .resolves();
            ...
            let res = await platform.updateOrderState(
                savedOrder.order,
                state);
        ...
        it('should not update order state', async () => {
            ...
            platform._platform = { internalCode: 10 };
            try {
                await platform.updateOrderState(
                    savedOrder,
                    state);
            } catch (error) {
    ...
    describe('fn(): updateNewsState()', function () {
        ...
        it('should udpate news state', async function () {
            const findOneAndUpdateStub = sandbox.stub(newsModel, 'findOneAndUpdate')
                .resolves();
            const idByCodStub = sandbox.stub(NewsTypeSingleton, 'idByCod')
                .withArgs(typeCod)
                .returns(typeId);
            let platform = new Platform();
            platform._platform = { internalCode: 10 };
    ...
    describe('fn(): isClosedRestaurant()', function () {
        it('should pass the validation', async function () {
            let platform = new Platform();
            platform._platform = { internalCode: 10, _id: '5d87cea59b0634004fd83c6b' };
    ...
    describe('fn(): saveOrders()', function () {
        const stateCod = 'rej_closed';
        const stateId = 2;
        ...
        it('should save the order', async function () {
            sandbox.stub(orderModel, 'find')
                .returns({ lean: () => Promise.resolve([]) });
            sandbox.stub(branchModel, 'find')
                .returns({ lean: () => Promise.resolve([branchThirdParty]) });

            sandbox.stub(Platform.prototype, 'saveNewOrders')
                .withArgs(thirdPartyOrders)
                .resolves(ordersProccessed);

            const stateIdByCodStub = sandbox.stub(NewsStateSingleton, 'stateByCod')
                .withArgs(stateCod)
                .returns(stateId);

            let res = await platform.saveOrders(thirdPartyOrders);

            expect(res).to.eql([{ id: 1, state: 'PENDING', branchId: 15 }]);
