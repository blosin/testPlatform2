...
import DispatchStrategy from '../../../../../../src/platforms/management/strategies/orderStrategy/dispatchStrategy';
...
describe('Dispatch Strategy.', function () {
    const newToSet = {
        typeId: 5,
    ...
    describe('fn(): manageNewType()', function () {
        ...
        beforeEach(() => {
            ...
            const optionsMock = {};
            createObjectStub = sandbox.stub(DispatchStrategy.prototype, 'createObjectsUpdate')
                .withArgs()
                .returns({ updateQuery: updateQueryMock, findQuery: findQueryMock, options: optionsMock });
            updateNewStub = sandbox.stub(DispatchStrategy.prototype, 'updateNew')
                .withArgs()
                .resolves();
        });
        afterEach(() => {
        ...
        it('should dispatch the order correctly', async function () {
            validateStub = sandbox.stub(DispatchStrategy.prototype, 'validateTransition')
                .withArgs()
                .returns(true);
            findNewStub = sandbox.stub(DispatchStrategy.prototype, 'findNew')
                .withArgs(newToSet.id)
                .resolves(newsSaved);
            const strategy = new DispatchStrategy(newToSet);
            strategy.platform = {
                dispatchOrder: () => { Promise.resolve() }
            };
