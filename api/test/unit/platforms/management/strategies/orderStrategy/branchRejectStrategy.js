...
import BranchRejectStrategy from '../../../../../../src/platforms/management/strategies/orderStrategy/branchRejectStrategy';
...
describe('BranchReject Strategy.', function () {
    const newToSet = {
        typeId: 2,
        newToSet: {
            rejectMessageId: 1,
            rejectMessageDescription: 'Descripcion rechazo',
            rejectMessageNote: 'Nota rechazo'
        },
    ...
    describe('fn(): manageNewType()', function () {
        ...
        beforeEach(() => {
            ...
            const optionsMock = {};
            createObjectStub = sandbox.stub(BranchRejectStrategy.prototype, 'createObjectsUpdate')
                .withArgs()
                .returns({ updateQuery: updateQueryMock, findQuery: findQueryMock, options: optionsMock });
            updateNewStub = sandbox.stub(BranchRejectStrategy.prototype, 'updateNew')
                .withArgs()
                .resolves();
        });
        afterEach(() => {
        ...
        it('should branchreject the order correctly', async function () {
            validateStub = sandbox.stub(BranchRejectStrategy.prototype, 'validateTransition')
                .withArgs()
                .returns(true);

            findNewStub = sandbox.stub(BranchRejectStrategy.prototype, 'findNew')
                .withArgs(newToSet.id)
                .resolves(newsSaved);
            let strategy = new BranchRejectStrategy(newToSet);
            strategy.platform = {
                branchRejectOrder: () => { Promise.resolve() }
            };

            strategy.trace = {
                entity: entity,
                update: {
                    rejectMessageId: newToSet.rejectMessageId,
                    rejectMessageDescription: newToSet.rejectMessageDescription,
                    rejectMessageNote: newToSet.rejectMessageNote
                }
            };
        ...
        it('should fail the validation transition', async function () {
            ...
            findNewStub = sandbox.stub(BranchRejectStrategy.prototype, 'findNew')
                .withArgs(newToSet.id)
                .resolves(newsSaved);
            const strategy = new BranchRejectStrategy(newToSet);
            strategy.platform = {
                branchRejectOrder: () => { Promise.resolve() }
            };
            try {
