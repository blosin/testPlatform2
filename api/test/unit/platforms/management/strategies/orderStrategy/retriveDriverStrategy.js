...
import RetrieveDriverStrategy from '../../../../../../src/platforms/management/strategies/orderStrategy/retriveDriverStrategy';
...
describe('RetriveDriver Strategy.', function () {
    const newToSet = {
        typeId: 12,
        id: "5d2dcea6c7632d4a9bfbe0e9",
    };
    const typeId = 12;
    const typeCod = 'upd_ord';
    ...
    describe('fn(): manageNewType()', function () {
        ...
        beforeEach(() => {
            ...
            const optionsMock = {};
            createObjectStub = sandbox.stub(RetrieveDriverStrategy.prototype, 'createObjectsUpdate')
                .withArgs()
                .returns({ updateQuery: updateQueryMock, findQuery: findQueryMock, options: optionsMock });
            updateNewStub = sandbox.stub(RetrieveDriverStrategy.prototype, 'updateNew')
                .withArgs()
                .resolves();
        });
        afterEach(() => {
        ...
        it('should retrieve the driver of order correctly', async function () {
            validateStub = sandbox.stub(RetrieveDriverStrategy.prototype, 'validateTransition')
                .withArgs()
                .returns(true);
            findNewStub = sandbox.stub(RetrieveDriverStrategy.prototype, 'findNew')
                .withArgs(newToSet.id)
                .resolves(newsSaved);
            sandbox.stub(news, 'updateOne')
                .returns({ lean: () => Promise.resolve({ ok: 1 }) });


            const strategy = new RetrieveDriverStrategy(newToSet);
            strategy.platform = {
                retriveDriver: () => { Promise.resolve() }
            };
