...
import DeliveryStrategy from '../../../../../../src/platforms/management/strategies/orderStrategy/deliveryStrategy';
...
describe('Delivery Strategy.', function () {
    const newToSet = {
        typeId: 14,
    ...
    describe('fn(): manageNewType()', function () {
        ...
        beforeEach(() => {
            ...
            const optionsMock = {};
            createObjectStub = sandbox.stub(DeliveryStrategy.prototype, 'createObjectsUpdate')
                .withArgs()
                .returns({ updateQuery: updateQueryMock, findQuery: findQueryMock, options: optionsMock });
            updateNewStub = sandbox.stub(DeliveryStrategy.prototype, 'updateNew')
                .withArgs()
                .resolves();
        });
        afterEach(() => {
        ...
        it('should delivery the order correctly', async function () {
            validateStub = sandbox.stub(DeliveryStrategy.prototype, 'validateTransition')
                .withArgs()
                .returns(true);
            findNewStub = sandbox.stub(DeliveryStrategy.prototype, 'findNew')
                .withArgs(newToSet.id)
                .resolves(newsSaved);
            const strategy = new DeliveryStrategy(newToSet);
            strategy.platform = {
                deliveryOrder: () => { Promise.resolve() }
            };
