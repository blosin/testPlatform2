...
import ConfirmStrategy from '../../../../../../src/platforms/management/strategies/orderStrategy/confirmStrategy';
...
describe('Confirm Strategy.', function () {
    const newToSet = {
        typeId: 5,
        deliveryTimeId: 1,
    ...
    describe('fn(): manageNewType()', function () {
        ...
        beforeEach(() => {
            ...
            const optionsMock = {};
            createObjectStub = sandbox.stub(ConfirmStrategy.prototype, 'createObjectsUpdate')
                .withArgs()
                .returns({ updateQuery: updateQueryMock, findQuery: findQueryMock, options: optionsMock });
            updateNewStub = sandbox.stub(ConfirmStrategy.prototype, 'updateNew')
                .withArgs()
                .resolves();
        });
        afterEach(() => {
        ...
        it('should confirm the order correctly', async function () {
            validateStub = sandbox.stub(ConfirmStrategy.prototype, 'validateTransition')
                .withArgs()
                .returns(true);
            findNewStub = sandbox.stub(ConfirmStrategy.prototype, 'findNew')
                .withArgs(newToSet.id)
                .resolves(newsSaved);
            const strategy = new ConfirmStrategy(newToSet);
            strategy.platform = {
                confirmOrder: () => { Promise.resolve() }
            };
            strategy.trace = { update: { deliveryTimeId: newToSet.deliveryTimeId } };
