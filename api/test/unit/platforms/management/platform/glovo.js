...
const platform = {
  ...
  credentials: {
    type: 'push',
    data: { token: '8163f4b0-74be-4286-9152-67d922bdaeb8' }
  }
};
...
describe('GLOVO management.', function () {
  ...
  describe('fn(): init()', function () {
    it('should init correctly', async function () {
      const glovo = new Glovo(platform);
      await glovo.init();
    });
  ...
  describe('fn(): getOrders()', function () {
    it('should create an order correctly', async function () {
      ...
      const saveNewOrdersStub = sandbox
        .stub(glovo, 'saveGlovoOrder')
        .resolves(newOrders);
      let saved = await glovo.saveGlovoOrder(
        orders,
        platformGlovo.credentials.data.token
      );
      expect(saved).to.eql(newOrders);
      expect(saveNewOrdersStub.callCount).to.equal(1);
      expect(
        saveNewOrdersStub.calledWith(
          orders,
          platformGlovo.credentials.data.token
        )
      ).to.be.true;
