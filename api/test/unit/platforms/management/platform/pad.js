...
const branches = [{
    ...
    "platforms": [
        {
            "progClosed": [{ close: new Date((2019, 8, 10, 10, 15)), open: new Date(2019, 8, 10, 10, 25), description: 'Closing test' }],
    ...
    describe('fn(): confirmOrder()', function () {
        ...
        it('should confirm order correctly', async function () {
            ...
            const stateIdByCodStub = sandbox.stub(NewsStateSingleton, 'stateByCod')
                .withArgs(stateCod)
                .returns(stateId);
            const updOrderStateStub = sandbox.stub(Pad.prototype, 'updateOrderState')
                .resolves(newOrders);
            const axiosStub = sandbox.stub(axios, 'post').resolves({ data: newOrders });
