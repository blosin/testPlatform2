...
describe('PEDIDOSYA management.', function () {
  ...
  beforeEach(() => {
    ...
    updateLastContactStub = sandbox
      .stub(PedidosYa.prototype, 'updateLastContact')
      .returns();
  });
  afterEach(() => {
    sandbox.restore();
  ...
  describe('fn(): interactWithOrders()', function () {
    ...
    it('should reject an order correctly by the platform', async function () {
      ...
      savedOrderStub.resolves(savedOrd);
      ...
      stateByCodStub.withArgs(rejClosedStateCod).returns(rejClosedState);
  ...
  describe('fn(): rejectOrder()', function () {
    ...
    it('should reject order correctly', async function () {
      ...
      const res = await py.branchRejectOrder(
        order,
        rejectMessageId,
        rejectMessageNote
      );
  ...
  describe('fn(): receiveOrder()', function () {
    it('should receive order correctly', async function () {
      ...
      const getBranchPlatformsStub = sandbox
        .stub(PedidosYa.prototype, 'getBranchPlatform')
  ...
  describe('fn(): closeRestaurant()', function () {
    ...
    it('should close the restaurant correctly', async function () {
      ...
      const res = await py.closeRestaurant(
        branches[0].branchId,
        branches[0].platforms[0],
        5,
        'Test close'
      );
    ...
    it('should fail closing the restaurant', async function () {
      ...
      try {
        ...
        expect(error).to.eql(
          `Failed to closeRestaurant. RestaurantCode: ${branches[0].branchId}.`
        );
      }
    });
  });
  ...
  describe('fn(): openRestaurant()', function () {
    ...
    it('should fail opening the restaurant', async function () {
      ...
      try {
        ...
        expect(error).to.eql(
          `Failed to openRestaurant. RestaurantCode: ${branches[0].branchId}.`
        );
      }
    });
  });
  ...
  describe('fn(): retriveDriver()', function () {
    ...
    it('should fail retriving the order driver', async function () {
      ...
      try {
        ...
        expect(error).to.eql(
          `Can not retrive order driver. BranchId ${order.id}.`
        );
      }
    });
  });
