...
describe('RAPPI management.', function () {
    ...
    beforeEach(() => {
        ...
        updateLastContactStub = sandbox.stub(Rappi.prototype, 'updateLastContact').returns();
    });
    afterEach(() => {
        sandbox.restore();
    ...
    describe('fn(): loginGetOrders()', function () {
        ...
        it('should login-get-orderds correctly', async function () {
            ...
            expect(getOrdersStub.calledWith(response['headers']['x-auth-int'])).to.be.true;
        });
    ...
    describe('fn(): getOrders()', function () {
        it('should create an order correctly', async function () {
            ...
            const saveNewOrdersStub = sandbox.stub(rappi, 'saveNewOrders')
                .resolves(newOrders);
            ...
            expect(saveNewOrdersStub.calledWith(orders, rappi._platform)).to.be.true;
        });
    ...
    describe('fn(): confirmOrder()', function () {
        ...
        it('should confirm order correctly', async function () {
            ...
            const axiosStub = sandbox.stub(axios, 'get').resolves({ data: newOrders });
            let saved = await rappi
        ...
        it('should not confirm order correctly, but resolves', async function () {
            ...
            const stateIdByCodStub = sandbox
                ...
                .withArgs(stateCod)
                .returns(stateId);
            const updOrderStateStub = sandbox.stub(Rappi.prototype, 'updateOrderState')
                .resolves(newOrders);

            const rejAutoStub = sandbox.stub(Rappi.prototype, 'rejectWrongOrderAutomatically')
                .withArgs(orders[0]._id)
                .resolves();

            const loginStub = sandbox.stub(Rappi.prototype, 'loginToRappi')
                .resolves('token');
            const axiosStub = sandbox
                .stub(axios, 'get')
                .rejects('Error+++');
    ...
    describe('fn(): branchRejectOrder()', function () {
        ...
        it('should reject order correctly', async function () {
            ...
            const res = await rappi.branchRejectOrder(orders[0], rejectMessageId, rejectMessageNote);
        ...
        it('should not reject order correctly', async function () {
            ...
            try {
                await rappi.branchRejectOrder(orders[0], rejectMessageId, rejectMessageNote);
