...
import Performance from '../../../../../src/platforms/management/platform/performance';
...
const platformThirdParty = {
  name: 'Performance',
  thirdPartyId: 'performance',
  thirdPartySecret: 'performance_Secret',
  internalCode: 7,
...
describe('Performance management.', function () {
  ...
  beforeEach(() => {
    ...
    sandbox.stub(Performance.prototype, 'updateLastContact').returns();
  });
  afterEach(() => {
    sandbox.restore();
  ...
  describe('fn(): confirmOrder()', function () {
    ...
    it('should confirm order correctly', async function () {
      ...
      const updOrderStateStub = sandbox
        .stub(Performance.prototype, 'updateOrderState')
        .resolves(newOrders);

      let saved = await performance.confirmOrder(orders[0]);
    ...
    it('should not confirm order correctly, but resolves', async function () {
      ...
      const saved = await performance.confirmOrder(orders[0]);
  ...
  describe('fn(): dispatchOrder()', function () {
    ...
    it('should dispatch order correctly', async function () {
      ...
      const updOrderStateStub = sandbox
        .stub(Performance.prototype, 'updateOrderState')
        .resolves(newOrders);
      let saved = await performance.dispatchOrder(orders[0]);
    ...
    it('should not dispatchOrder order correctly, but resolves', async function () {
      ...
      const updOrderStateStub = sandbox
        .stub(Performance.prototype, 'updateOrderState')
        .resolves(newOrders);
      const saved = await performance.dispatchOrder(orders[0]);
  ...
  describe('fn(): deliveryOrder()', function () {
    ...
    it('should delivery order correctly', async function () {
      ...
      let saved = await performance.deliveryOrder(orders[0]);
    ...
    it('should not deliveryOrder order correctly, but resolves', async function () {
      ...
      const saved = await performance.deliveryOrder(orders[0]);
