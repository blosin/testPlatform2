...
describe('Validations Branch schema.', function () {
    it('with correct minimum data', function (done) {
        const branch = new Branch({
            ...
            platforms: [{
                ...
                progClosed: [{ close: new Date(), open: new Date(), description: 'descripcion' }]
            }],
            tzo: -3
        });
    ...
    it('test attributes larger than allowed', function (done) {
        ...
        expect(err.errors['smartfran_sw.agent.installedVersion'].name).to.equal('ValidatorError');
        expect(err.errors['smartfran_sw.notificator.installedVersion'].name).to.equal('ValidatorError');
        done();
    });
    it('test cast errors', function (done) {
        ...
        expect(err.errors['platforms.0.branchIdReference'].name).to.equal('CastError');
        expect(err.errors['platforms.0.lastGetNews'].name).to.equal('CastError');
        expect(err.errors['platforms.0.progClosed.0.close'].name).to.equal('CastError');
        expect(err.errors['platforms.0.progClosed.0.open'].name).to.equal('CastError');
