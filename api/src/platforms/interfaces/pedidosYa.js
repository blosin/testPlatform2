	...
	newsFromOrders: function (data, platform, newsCode, stateCod, branch, uuid) {
		...
		return new Promise((resolve, reject) => {
			...
			const paymentenMapper = (payment, discounts, thirdParty) => {
				try {
					...
					if (!payment.online) {
						...
						paymentNews.remaining = payment.paymentAmount - payment.subtotal + paymentNews.discount;			/* Vuelto - Si el pago es efectivo, calculo vuelto*/
						paymentNews.partial = payment.paymentAmount;														/* Con cuanto paga - Si el pago es efectivo, indica con cuanto paga*/
					}
					else if (!!payment.card && !!payment.card.operationType && !!paymentType[payment.card.operationType])
						paymentNews.typeId = paymentType[payment.card.operationType].paymentId;
					else
			...
			const detailsMapper = (order) => {
				try {
					...
					for (let detail of order.details) {
						let det = {};
						if (detail.product.section.integrationName.trim().toLowerCase() == 'promo') {
							let detHeader = {};
								...
								for (let optionsGroups of detail.optionGroups) {
									...
									for (let options of optionsGroups.options) {
										if (options.integrationCode &&
											...
											optionsString += ' ' + options.name + ' cantidad ' + options.quantity;
										}
									}
								}
							}
			...
			try {
				...
				news.order.payment = paymentenMapper(data.order.payment, data.order.discounts, data.thirdParty);
				news.order.driver = driverMapper(data.driver);
				news.extraData = extraDataMapper(branch, platform);
				news.order.totalAmount = data.order.payment.subtotal - news.order.payment.discount;
