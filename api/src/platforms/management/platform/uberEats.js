...
class UberEats extends Platform {
    constructor(platform) {
        ...
        this.urlGetOrders = 'https://api.uber.com/v1/eats/stores/' + this._platform.credentials.data.store_id + '/created-orders';
    ...
    loginGetOrders(scope) {
        this.loginToUberEats(scope)
            .then((auth) => {
                ...
                new CustomError(APP_PLATFORM.LOGIN, msg, this.uuid, { error: error.toString() });
            });
    }
    ...
    loginToUberEats(scope) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (!response)
                    throw 'Can not login to uberEats.'
                this.updateLastContact();
                resolve(response.data.access_token);
            } catch (error) {
                if (!error) error = '';
                const msg = 'Failed to login.';
                const err = new CustomError(APP_PLATFORM.LOGIN, msg, this.uuid, { error: error.toString() });
                reject(err);
            }
        });
    }
    ...
    getOrders(token) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let orderArray = [];
                ordersDetails.forEach(singleOrder => orderArray.push(singleOrder.data));
                this.saveNewOrders(orderArray)
                    .then(() => this.loginToUberEats("eats.order")
                        .then((confirmToken) => {
                            if (orderArray.length > 0) this.sendPosConfirmation(orderArray, confirmToken);
                        }
                        ))
                ...
                const
                    err = new CustomError(APP_PLATFORM.GETORD, msg, this.uuid, { error: error.toString() });
                reject(err);
            }
        });
    }
    ...
    async sendPosConfirmation(orders, token) {
        try {
            ...
            let orderConfirmationPromise = orders.map(order => {
                const urlToConfirm = "https://api.uber.com/v1/eats/orders/" + order.id + "/accept_pos_order";
            ...
            const err = new CustomError(APP_PLATFORM.GETORD, msg, this.uuid, { error: error.toString() });
            return err;
        }
    }
