...
const find = async (model, req, res) => {
  try {
    ...
    if (queryParams.sort) sort = JSON.parse(queryParams.sort);
    if (queryParams._filters) filter = JSON.parse(queryParams._filters);
    if (queryParams._populates) populates = JSON.parse(queryParams._populates);
    if (queryParams._select) select = JSON.parse(queryParams._select);
...
const login = async (
  ...
  try {
    ...
    if (!!user) {
      const accessToken = jwt.sign({ user }, settings.token.secret, {
        expiresIn
      });
